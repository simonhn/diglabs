...
Capistrano::Configuration.instance.load do
  ...
  namespace :db do
    ...
    task :setup, :except => { :no_release => true } do
      ...
      location = '/Users/simonnielsen/Documents/Code/diglabs/pav/config/settings.yml.erb'
      #location = fetch(:template_dir, "config") + '/settings.yml.erb'
      template = File.file?(location) ? File.read(location) : default_template
      
      config = ERB.new(template)

      run "mkdir -p #{shared_path}/pav/conf" 
      run "mkdir -p #{release_path}/pav/conf" 
      put config.result(binding), "#{shared_path}/pav/conf/settings.yml"
    end
      ...
      task :symlink, :except => { :no_release => true } do
        run "ln -nfs #{shared_path}/pav/conf/settings.yml #{release_path}/pav/conf/settings.yml" 
      end
